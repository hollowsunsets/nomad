...
// JobPaginationIterator is a wrapper over a go-memdb iterator that implements
// the paginator Iterator interface.
type JobPaginationIterator struct {
	iter          memdb.ResultIterator
	byCreateIndex bool
}

func (it JobPaginationIterator) Next() (string, interface{}) {
	raw := it.iter.Next()
	if raw == nil {
		return "", nil
	}
	...
	if it.byCreateIndex {
		token = fmt.Sprintf("%v-%v", job.CreateIndex, job.ID)
	}
...
func (j *Job) List(args *structs.JobListRequest, reply *structs.JobListResponse) error {
	...
	opts := blockingOptions{
		...
		run: func(ws memdb.WatchSet, store *state.StateStore) error {
			...
			var jobIter JobPaginationIterator
			...
			paginator, err := state.NewPaginator(jobIter, args.QueryOptions,
